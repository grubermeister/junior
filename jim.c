...
static AioFile *JimMakeChannel(Jim_Interp *interp, FILE *fh, int fd, Jim_Obj *filename, const char *hdlfmt, int family, const char *mode);
...
static int aio_eventinfo(Jim_Interp *interp, AioFile * af, unsigned mask, Jim_Obj **scriptHandlerObj, int argc, Jim_Obj * const *argv) {
    if (argc == 0) {
        if (*scriptHandlerObj)
    ...
    *scriptHandlerObj = argv[0];
    Jim_CreateFileHandler(interp, af->fd, mask, JimAioFileEventHandler, scriptHandlerObj, JimAioFileEventFinalizer);
...
static int JimAioOpenCommand(Jim_Interp *interp, int argc, Jim_Obj *const *argv) {
    const char *mode;
